...
const Home: React.FC = () => {
  ...
  const { bookmarks, toggleBookmark, theme, isLoggedIn } = useAppStore();
  const { user } = useAuth();
    ...
    <>
      <main className="min-h-screen">
        ...
        <section className="max-w-7xl mx-auto px-6 pt-28 pb-12 flex flex-col h-[1088px]">
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3 flex-1 h-[760px]">
              <div className="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" />
              <p className="text-xs text-muted-foreground font-medium">
                Sedang mengambil data lowongan...
              </p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 h-[760px] sm:grid-cols-2 lg:grid-cols-3 gap-5 content-start">
                    ...
                    >
                      <div>
                        ...
                        <div className="space-y-1 mb-3">
                          ...
                          <div className="text-xs text-muted-foreground font-medium flex items-center gap-1 overflow-hidden">
                            <span className="truncate shrink min-w-0">
                              {job.company}
                            </span>
                            {job.isVerified && (
                              <ShieldCheck className="h-3.5 w-3.5 text-blue-500 fill-blue-500/10 shrink-0" />
                            )}
                            <span className="truncate shrink min-w-0">
                              • {job.location}
                            </span>
