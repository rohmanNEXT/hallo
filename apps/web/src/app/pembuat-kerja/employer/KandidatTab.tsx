...
const CircleCheckIcon = () => <CheckCircle className="w-4 h-4 text-slate-400 dark:text-slate-500 shrink-0" />;
...
const DEFAULT_POSITIONS = Array.from({ length: 400 }, (_, i) => {
  const baseRoles = [
    ...
    "Product Manager", "Data Scientist", "DevOps Engineer", "QA Engineer", "Mobile Developer",
    "Digital Marketer", "SEO Specialist", "Content Writer", "Social Media Specialist",
    "Sales Executive", "Business Development", "Accountant", "HR Specialist", "Customer Service",
    "Office Admin", "Graphic Designer", "Video Editor", "IT Support", "Network Engineer",
    "Security Analyst", "Database Administrator", "Project Manager", "Scrum Master", "Business Analyst"
  ];
  const prefixes = ["Junior", "Senior", "Lead", "Staff", "Principal", "Associate", "Trainee", "Expert"];
...
const DEFAULT_SKILLS = Array.from({ length: 900 }, (_, i) => {
  const baseSkills = [
    "React", "Node.js", "TypeScript", "JavaScript", "Python", "SQL", "Java", "C#", "C++",
    "Go", "Rust", "Swift", "Kotlin", "HTML5", "CSS3", "Sass", "Tailwind CSS", "Bootstrap",
    "Next.js", "Vue.js", "Angular", "Express", "NestJS", "Django", "Flask", "Spring Boot",
    "PostgreSQL", "MongoDB", "MySQL", "Redis", "Elasticsearch", "Docker", "Kubernetes", "AWS",
    "Azure", "GCP", "Git", "GitHub", "GitLab", "CI/CD", "Jira", "Confluence", "Figma", "Adobe Photoshop",
    "Adobe Illustrator", "Premiere Pro", "After Effects", "Sketch", "InVision", "Excel", "Word",
    "PowerPoint", "Google Analytics", "Google Ads", "Facebook Ads", "SEO", "SEM", "Copywriting",
    "Public Speaking", "Negotiation", "Project Management", "Agile", "Scrum", "Kanban", "Leadership"
  ];
  const suffix = ["Basic", "Intermediate", "Advanced", "Expert", "Proficient", "Certified", "Practitioner"];
...
export function calculateHalloScore(talent: any, job: any) {
  ...
  if (talent?.title && job?.title) {
    ...
    const hasMatch = talent.title.toLowerCase().split(/\s+/).some((w: string) => jobWords.includes(w));
    if (hasMatch) score += 1;
    else score += 0.5;
...
const KandidatTab: React.FC = () => {
  ...
  const [openMatchAccordions, setOpenMatchAccordions] = useState<Record<string, boolean>>({
    posisi: true,
    ringkasan: false,
    pendidikan: false,
    pengalaman: false,
  ...
  const enrichedApplications = filteredAppsByRole.map((app) => {
  ...
  const filteredApplications = enrichedApplications.filter((app) => {
    ...
    if (selectedProvince) {
      const matchProv = app.talent?.location.toLowerCase().includes(selectedProvince.toLowerCase());
      if (!matchProv) {
    ...
    if (selectedRegency && !app.talent?.location.toLowerCase().includes(selectedRegency.toLowerCase())) {
      return false;
    }
    ...
    // Education Filter
    if (cEducation !== 'Semua') {
      if (cEducation === 'SMA/SMK' && app.talent?.education !== 'SMA/SMK') return false;
    ...
    if (candShowFav && app.talentId && !favoriteTalents.includes(app.talentId)) {
      return false;
    }
    if (candShowUnchecked && app.status !== 'Melamar') return false;
    if (candShowWithPhoto && (!app.talent?.avatar || app.talent?.avatar.includes('default'))) return false;
    if (candShowByExp) {
      const isSesuai = (app.talent?.experienceYears || 0) >= (app.job?.title?.includes('Senior') ? 5 : 1);
          ...
          <Button
            onClick={() => {
              if (user?.plan !== 'Platinum') {
                alert("Fitur Export ke PDF hanya tersedia untuk pengguna Platinum.");
                return;
              }
              window.print();
            }}
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between -mt-12 sm:-mt-15 gap-4 mb-4">
              <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                <div className="relative h-24 w-24 md:h-30 md:w-30 rounded-2xl overflow-hidden border-4 border-card shadow-md bg-muted shrink-0">
                   {viewingTalentProfile.avatar && !viewingTalentProfile.avatar.includes('default-avatar') && !viewingTalentProfile.avatar.includes('placeholder') ? (
                    <>
                      <Image
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <p className="text-xs text-muted-foreground font-semibold leading-relaxed">
                {viewingTalentProfile.aboutMe || "Saya adalah Frontend Engineer yang bersemangat dalam membangun antarmuka web yang interaktif, responsif, dan ramah pengguna."}
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 bg-background/40 border border-border/50 p-3 rounded-xl">
                  <span className="text-sm shrink-0"><BriefcaseIcon className="w-4 h-4 shrink-0 text-slate-400" /></span>
                  <span className="text-xs font-semibold text-foreground">
                    Junior Developer di TechCorp Indonesia ({viewingTalentProfile.experienceYears || 1} Tahun)
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 bg-background/40 border border-border/50 p-3 rounded-xl">
                  <span className="text-sm shrink-0"><GradCapIcon className="w-4 h-4 shrink-0 text-slate-400" /></span>
                  <span className="text-xs font-semibold text-foreground">
                    {viewingTalentProfile.education || 'S1 Teknik Informatika'} - Universitas Indonesia (2018 - 2022)
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                {(viewingTalentProfile.skills && viewingTalentProfile.skills.length > 0
                  ? viewingTalentProfile.skills
                  : ['React', 'Next.js', 'TypeScript', 'Tailwind CSS', 'Zustand', 'HTML5', 'CSS3']
                ).map((sk: string) => (
                  <span key={sk} className="text-[12px] font-semibold text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-stone-700 px-3 py-1 rounded-full bg-transparent">
                    {sk}
                  </span>
                ))}
              </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 bg-background/40 border border-border/50 p-3.5 rounded-xl">
                  <span className="text-xs shrink-0"><AwardIcon className="w-3.5 h-3.5 shrink-0 text-slate-400" /></span>
                  <span className="text-xs font-semibold text-foreground">AWS Certified Developer - Associate</span>
                </div>
                <div className="flex items-center gap-3 bg-background/40 border border-border/50 p-3.5 rounded-xl">
                  <span className="text-xs shrink-0"><AwardIcon className="w-3.5 h-3.5 shrink-0 text-slate-400" /></span>
                  <span className="text-xs font-semibold text-foreground">Next.js Professional Certificate</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 bg-background/40 border border-border/50 p-3.5 rounded-xl">
                  <span className="text-xs shrink-0"><UserIcon className="w-3.5 h-3.5 shrink-0 text-slate-400" /></span>
                  <span className="text-xs font-semibold text-foreground">Ketua Himpunan Mahasiswa Informatika (2020 - 2021)</span>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <div className="space-y-2.5 my-1">
                      <div className="flex items-center gap-2 text-xs font-medium text-foreground/80">
                        <CircleCheckIcon />
                        <span>{viewingTalentProfile.title || 'Frontend Developer'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs font-medium text-foreground/80">
                        <CircleCheckIcon />
                        <span>{viewingTalentProfile.title?.includes('Frontend') ? 'Full Stack Developer' : 'Web Developer'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-xs font-medium text-foreground/80">
                        <CircleCheckIcon />
                        <span>{viewingTalentProfile.title?.includes('Designer') ? 'UI Developer' : 'Software Engineer'}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-2 md:gap-4 items-start pt-2 border-t border-border/45">
                  ...
                  <span className="text-sm font-bold text-foreground">
                    IDR {((viewingTalentProfile.expectedSalary || 5000000) / 1000000 - 4).toFixed(0)} jt - {((viewingTalentProfile.expectedSalary || 5000000) / 1000000).toFixed(0)} jt
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-2 md:gap-4 items-start pt-2 border-t border-border/45">
                  ...
                  <div className="flex items-center gap-2 text-xs font-medium text-foreground/80">
                    <CircleCheckIcon />
                    <span>{viewingTalentProfile.location || 'Surabaya'}, Jawa Timur</span>
                  </div>
                </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-4 pt-1 text-xs">
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><UserIcon className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Nama Lengkap</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.name}</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><UserIcon className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Nama Panggilan</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.nickname || viewingTalentProfile.name.split(' ')[0]}</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><BriefcaseIcon className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Posisi</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.title}</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><Phone className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Whatsapp</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.waNumber || '081234567890'}</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><Mail className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Email</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.email || `${viewingTalentProfile.name.toLowerCase().replace(/\s+/g, '')}@example.com`}</span>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-slate-400 mt-0.5"><MapPin className="w-3.5 h-3.5 shrink-0" /></span>
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Tempat Tinggal</span>
                    <span className="font-bold text-foreground">{viewingTalentProfile.location || 'Jakarta Pusat'}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative">
              ...
              <div className="space-y-4 pt-1 text-xs">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Resume</span>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="font-bold text-foreground truncate max-w-[150px]">Portofolio_Frontend.pdf</span>
                      <span className="bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 text-[8px] rounded-md font-bold text-slate-600 dark:text-slate-350 shrink-0">PDF</span>
                      <a 
                        href="#" 
                        onClick={(e) => {
                          e.preventDefault();
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Website</span>
                    <a href="#" className="font-bold text-primary hover:underline block truncate max-w-[180px] mt-0.5">
                      https://{viewingTalentProfile.website || 'budisantoso.dev'}
                    </a>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">LinkedIn</span>
                    <a href="#" className="font-bold text-primary hover:underline block truncate max-w-[180px] mt-0.5">
                      https://linkedin.com/in/{viewingTalentProfile.name.toLowerCase().replace(/\s+/g, '')}
                    </a>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <div className="space-y-3 pt-1">
                {[
                  { label: 'CV Lamaran', value: viewingTalentProfile.hasCv ? 'Tersedia' : 'Tidak Tersedia' },
                  { label: 'Ekspektasi Gaji', value: `Rp ${viewingTalentProfile.expectedSalary?.toLocaleString('id-ID') || '18.500.000'}` },
                  { label: 'Kapan Mulai Kerja', value: viewingTalentProfile.readyNow ? 'Siap Segera' : '1 Bulan Notice' },
                  { label: 'Lokasi Kerja', value: 'Onsite' },
                  { label: 'Prestasi & Sertifikat', value: viewingTalentProfile.hasCertificate ? 'Tersedia' : 'Tidak Tersedia' },
                  { label: 'Perangkat Pendukung', value: viewingTalentProfile.hasLaptop ? 'Laptop Pribadi, HP' : 'HP' },
                  { label: 'Kendaraan Pribadi', value: viewingTalentProfile.hasMotor ? 'Motor' : '-' },
                  { label: 'Dokumen Pendukung', value: [viewingTalentProfile.hasSim ? 'SIM' : '', viewingTalentProfile.hasSkck ? 'SKCK' : ''].filter(Boolean).join(', ') || '-' },
                ].map((item, idx) => (
                  <div key={idx} className="flex items-start gap-2 text-xs leading-relaxed">
                    <Check className="w-4 h-4 shrink-0 mt-0.5" />
                    <p className="font-semibold text-slate-700 dark:text-slate-300">
                      {item.label}: <span className="text-muted-foreground font-semibold">{item.value}</span>
                    </p>
                  </div>
                ))}
              </div>
      ...
      <div className="grow flex flex-col justify-start">
                      ...
                      <div className="space-y-2">
                          ...
                          <button
                            ...
                            onClick={() =>
                              setPopoverActiveGroup(
                                popoverActiveGroup === 'posisi' ? null : 'posisi',
                              )
                            }
                          ...
                          {popoverActiveGroup === 'posisi' && (
                            <div className="pt-2 space-y-2">
                              ...
                              <div className="max-h-32 overflow-y-auto space-y-1.5 pr-1 border rounded-lg p-2 bg-background/50">
                                {(() => {
                                  const filtered = DEFAULT_POSITIONS.filter((pos) =>
                                    pos.toLowerCase().includes(positionSearchQuery.toLowerCase()),
                                  );
                                  if (filtered.length === 0 && positionSearchQuery.trim() !== '') {
                                    const customPos = positionSearchQuery.trim();
                                    const isSelected = selectedPositions.includes(customPos);
                                    return (
                                      <div className="space-y-1">
                                        <p className="text-[10px] text-muted-foreground italic">Posisi tidak ditemukan.</p>
                                        <button
                                          type="button"
                                          onClick={() => {
                                            if (isSelected) {
                                              setSelectedPositions(selectedPositions.filter((x) => x !== customPos));
                                            } else {
                                              setSelectedPositions([...selectedPositions, customPos]);
                                            }
                                            setPositionSearchQuery('');
                                          }}
                                        ...
                                        >
                                          ...
                                          <span className="text-[10px] bg-primary text-primary-foreground px-1 py-0.5 rounded">Tambah</span>
                                        </button>
                                      </div>
                                    );
                                  }
                                  return filtered.slice(0, 50).map((pos) => {
                                    ...
                                    return (
                                      <label key={pos} className="flex items-center gap-2 text-[12px] cursor-pointer select-none">
                                        <input
                                          type="checkbox"
                                          checked={isSelected}
                                          onChange={() => {
                                            if (isSelected) {
                                              setSelectedPositions(selectedPositions.filter((x) => x !== pos));
                                            } else {
                              ...
                              {selectedPositions.length > 0 && (
                                <div className="flex flex-wrap gap-1 pt-1 max-h-20 overflow-y-auto">
                                  {selectedPositions.map((pos) => (
                                    <Badge key={pos} variant="secondary" className="text-[10px] py-0 px-1.5 flex items-center gap-1 font-bold">
                                      <span>{pos}</span>
                                      <X
                                        className="w-2.5 h-2.5 cursor-pointer text-muted-foreground hover:text-foreground"
                                        onClick={() => setSelectedPositions(selectedPositions.filter((x) => x !== pos))}
                                      />
                                    </Badge>
                                  ))}
                                </div>
                            ...
                            <div className="pt-2 space-y-2">
                              ...
                              <div className="max-h-32 overflow-y-auto space-y-1.5 pr-1 border rounded-lg p-2 bg-background/50">
                                {(() => {
                                  ...
                                  if (filtered.length === 0 && skillSearchQuery.trim() !== '') {
                                    const customSkill = skillSearchQuery.trim();
                                    const isSelected = selectedSkills.includes(customSkill);
                                    return (
                                      <div className="space-y-1">
                                        <p className="text-[10px] text-muted-foreground italic">Skill tidak ditemukan.</p>
                                        <button
                                          type="button"
                                          onClick={() => {
                                            if (isSelected) {
                                              setSelectedSkills(selectedSkills.filter((x) => x !== customSkill));
                                            } else {
                                  ...
                                  return filtered.slice(0, 50).map((sk) => {
                                    ...
                                    return (
                                      <label key={sk} className="flex items-center gap-2 text-[12px] cursor-pointer select-none">
                                        <input
                                          type="checkbox"
                                          checked={isSelected}
                                          onChange={() => {
                                            if (isSelected) {
                                              setSelectedSkills(selectedSkills.filter((x) => x !== sk));
                                            } else {
                              ...
                              {selectedSkills.length > 0 && (
                                <div className="flex flex-wrap gap-1 pt-1 max-h-20 overflow-y-auto">
                                  {selectedSkills.map((sk) => (
                                    <Badge key={sk} variant="secondary" className="text-[10px] py-0 px-1.5 flex items-center gap-1 font-bold">
                                      <span>{sk}</span>
                                      <X
                                        className="w-2.5 h-2.5 cursor-pointer text-muted-foreground hover:text-foreground"
                        ...
                        <div className="border-b border-border/40 pb-2">
                          <button
                            ...
                            onClick={() =>
                              setPopoverActiveGroup(
                                popoverActiveGroup === 'pendidikan' ? null : 'pendidikan',
                              )
                            }
                            className="w-full flex items-center justify-between text-xs font-bold text-foreground py-1 cursor-pointer"
                          >
                            ...
                            <div className="pt-2 flex flex-wrap gap-1.5">
                              {[
                                ...
                                <button
                                  ...
                                  type="button"
                                  onClick={() => {
                                    if (edu === 'Semua Pendidikan') setCEducation('Semua');
                                    else if (edu.startsWith('S3')) setCEducation('S3');
                                    else if (edu.startsWith('S2')) setCEducation('S2');
                                    else if (edu.startsWith('S1')) setCEducation('S1');
                                    else if (edu.startsWith('D4')) setCEducation('D4');
                                    else setCEducation(edu);
                                  }}
                                  className={`px-2.5 py-1.5 text-[10px] rounded-lg border transition-all cursor-pointer font-bold ${
                                    (cEducation === 'Semua' && edu === 'Semua Pendidikan') ||
                                    (cEducation === 'S3' && edu.startsWith('S3')) ||
                                    (cEducation === 'S2' && edu.startsWith('S2')) ||
                                    (cEducation === 'S1' && edu.startsWith('S1')) ||
                                    (cEducation === 'D4' && edu.startsWith('D4')) ||
                                    (cEducation === edu && edu !== 'Semua Pendidikan' && !edu.startsWith('S3') && !edu.startsWith('S2') && !edu.startsWith('S1') && !edu.startsWith('D4'))
                        ...
                        <div className="border-b border-border/40 pb-2">
                          <button
                            ...
                            onClick={() =>
                              setPopoverActiveGroup(
                                popoverActiveGroup === 'gender' ? null : 'gender',
                              )
                            }
                            className="w-full flex items-center justify-between text-xs font-bold text-foreground py-1 cursor-pointer"
                          >
                        ...
                        <div className="border-b border-border/40 pb-2">
                          <button
                            ...
                            onClick={() =>
                              setPopoverActiveGroup(
                                popoverActiveGroup === 'kecamatan' ? null : 'kecamatan',
                              )
                            }
                            className="w-full flex items-center justify-between text-xs font-bold text-foreground py-1 cursor-pointer"
                          >
                          ...
                          {popoverActiveGroup === 'kecamatan' && (
                            <div className="pt-2 space-y-2 relative">
                              <div className="relative">
                                ...
                                >
                                  <span>{selectedProvince || 'Pilih Provinsi'}</span>
                                </button>
                                <ChevronDown className={`absolute right-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none transition-transform duration-200 ${isProvinceOpen ? 'rotate-180' : ''}`} />
                                {isProvinceOpen && (
                              ...
                              <div className="relative">
                                ...
                                >
                                  <span>{selectedRegency || 'Pilih Kabupaten/Kota'}</span>
                                </button>
                                <ChevronDown className={`absolute right-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground pointer-events-none transition-transform duration-200 ${isRegencyOpen ? 'rotate-180' : ''}`} />
                                {isRegencyOpen && selectedProvince && (
                                  <div className="absolute top-full left-0 right-0 mt-1 z-105 rounded-lg border bg-popover shadow-xl overflow-hidden text-popover-foreground">
                                    ...
                                    <div className="max-h-32 overflow-y-auto">
                                      ...
                                      {(
                                        PROVINCES.find((p) => p.province === selectedProvince)?.regencies || []
                                      )
                                        .filter((r) => r.toLowerCase().includes(regencySearch.toLowerCase()))
                                        .map((r) => (
                                          <button
                                            key={r}
                                            type="button"
        ...
        <div className="flex bg-zinc-100 dark:bg-zinc-800/80 p-1 rounded-xl items-center grow mt-4 gap-1">
          {(['Melamar', 'Terseleksi', 'Diterima', 'Ditutup'] as const).map((tab) => (
            ...
            >
              {tab} ({enrichedApplications.filter((app) => app.status === tab).length})
            </button>
          ))}
        </div>
            ...
            <div className="h-[600px] overflow-y-auto pr-1.5 scroll-smooth mt-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-2">
                {paginatedApplications.map((app) => (
                  <Card
                    key={app.id}
                    className={`border transition-all hover:border-primary/40 rounded-2xl overflow-hidden shadow-sm bg-card text-card-foreground hover:shadow-md duration-200 h-auto min-h-[140px] flex flex-col justify-between p-4 relative ${
                      selectedApplicationId === app.id
                        ? 'border-primary ring-1 ring-primary/20 bg-primary/2'
                        : 'border-border'
                    }`}
                    ...
                    <CardContent className="p-0 flex flex-col justify-between h-full gap-3.5">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3 min-w-0">
                          {app.talent?.avatar && !app.talent.avatar.includes('default-avatar') && !app.talent.avatar.includes('placeholder') ? (
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 min-w-0">
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  if (app.talentId) toggleFavoriteTalent(app.talentId);
                                }}
                                className="p-0.5 rounded-full hover:bg-muted text-muted-foreground hover:text-amber-500 transition-all border-none cursor-pointer shrink-0"
                              >
                                <Star
                                  className={`h-3.5 w-3.5 ${
                                    app.talentId && favoriteTalents.includes(app.talentId)
                            ...
                            <p className="text-[10.5px] font-bold text-primary truncate max-w-[120px] sm:max-w-[150px] mt-0.5">
                              {app.talent?.title}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="border-t border-border/50 pt-2.5 flex items-center justify-between gap-2 mt-auto">
                        <div className="flex items-center gap-1.5 min-w-0 text-[10.5px] text-muted-foreground">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <Badge
                            ...
                            className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 hover:bg-emerald-500/20 border-none font-bold text-[9px] px-2 py-0.5 rounded-full cursor-pointer transition-all"
                          >
                          ...
                          <button
                            ...
                            className="p-1 rounded-lg hover:bg-muted text-muted-foreground hover:text-primary transition-all cursor-pointer border border-border flex items-center justify-center bg-transparent"
            ...
            <Card className="relative w-full max-w-md bg-card border border-border rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col z-10 animate-in zoom-in-95 duration-150 text-foreground overflow-hidden">
              ...
              <div className="space-y-5">
                <div className="flex items-center gap-4">
                  {selectedDetailApp.talent?.avatar && !selectedDetailApp.talent.avatar.includes('default-avatar') && !selectedDetailApp.talent.avatar.includes('placeholder') ? (
                    <div className="h-14 w-14 rounded-full overflow-hidden border border-border shadow-xs relative shrink-0">
                      <Image
                      ...
                      />
                      <div className="avatar-fallback hidden absolute inset-0 items-center justify-center bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 text-lg font-black uppercase">
                        {selectedDetailApp.talent?.name?.[0] || '?'}
                      </div>
                    </div>
                  ) : (
                    <div className="h-14 w-14 rounded-full bg-slate-100 dark:bg-slate-800 border border-border flex items-center justify-center text-slate-700 dark:text-slate-200 text-lg font-black uppercase shrink-0">
                      {selectedDetailApp.talent?.name?.[0] || '?'}
                    </div>
                  )}
                  <div>
                ...
                <div className="flex justify-between items-center border-t border-border/60 pt-4 mt-2">
                  <button
                    onClick={() => {
                      setViewingTalentProfile(selectedDetailApp.talent);
                  ...
                  >
                    Lihat Profil <span className="text-primary font-black"><ArrowRight className="w-3.5 h-3.5 ml-1 inline-block" /></span>
                  </button>
                  <button
                    onClick={() => {
                      setSelectedDetailApp(null);
                    ...
                    className="text-xs font-bold text-muted-foreground hover:text-foreground cursor-pointer flex items-center gap-1 bg-transparent border-none p-0"
                  >
                    Chat Kandidat <span className="text-primary font-black"><ArrowRight className="w-3.5 h-3.5 ml-1 inline-block" /></span>
                  </button>
                </div>
              </div>
            </Card>
            ...
            <Card className="relative w-full max-w-lg bg-card/95 border border-border/70 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col z-10 animate-in zoom-in-95 duration-150 text-foreground overflow-hidden max-h-[85vh] backdrop-saturate-150">
              ...
              <div className="space-y-5 flex flex-col h-full overflow-hidden">
                <div className="shrink-0 space-y-1">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-primary/10 text-primary font-bold text-[10px] tracking-wider uppercase">
                    <><Sparkles className="w-4 h-4 inline-block mr-1 text-amber-500" /> Match Analysis</>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <span>Kandidat:</span>
                    <span className="font-bold text-foreground">{selectedMatchApp.talent?.name}</span>
                  </div>
                  
                  {/* Modern Score representation */}
                  <div className="flex items-center gap-4 mt-3 bg-muted/40 p-3 rounded-2xl border border-border/30">
                    <div className="text-2xl font-black text-primary tracking-tight shrink-0">
                      {calculateHalloScore(selectedMatchApp.talent, selectedMatchApp.job)}%
                    </div>
                    <div className="grow space-y-1">
                      ...
                      <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          ...
                          style={{ width: `${calculateHalloScore(selectedMatchApp.talent, selectedMatchApp.job)}%` }}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto pr-1 space-y-3 smooth-scroll">
                  {/* 1. Posisi */}
                  {(() => {
                    const match = selectedMatchApp.talent?.title && selectedMatchApp.job?.title && selectedMatchApp.talent.title.toLowerCase().split(/\s+/).some((w: string) => selectedMatchApp.job.title.toLowerCase().includes(w));
                    const score = match ? 100 : 100;
                    const isOpen = !!openMatchAccordions.posisi;
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        <button
                          type="button"
                          onClick={() => toggleMatchAccordion('posisi')}
                          className="w-full flex items-center justify-between p-4 text-xs font-bold text-foreground transition-colors"
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Posisi ({score}%)</span>
                          </span>
                        ...
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              ...
                              <p className="font-extrabold text-foreground">{selectedMatchApp.talent?.title || 'Frontend Developer'}</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Ringkasan Profil (100%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">Profil telah dilengkapi</p>
                            </div>
                          </div>
                        )}
                      </div>
                  ...
                  {/* 3. Pendidikan */}
                  {(() => {
                    const hasEdu = selectedMatchApp.talent?.education || 'S1 Teknik Informatika';
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Pendidikan (90%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">{hasEdu}</p>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Pengalaman Kerja (85%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">{exp} tahun {selectedMatchApp.talent?.title || 'Frontend Developer'}</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                      ...
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">Bahasa Indonesia</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">Bahasa Inggris</p>
                            </div>
                          </div>
                        )}
                      </div>
                      ...
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">{loc}</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Opsi Kerja (100%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">{opt}</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">Hybrid</p>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Portofolio (100%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">GitHub tersedia</p>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">Website portofolio tersedia</p>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    return (
                      <div className={`border rounded-2xl overflow-hidden transition-all duration-200 ${isOpen ? 'border-primary/30 shadow-sm bg-muted/20' : 'border-border/50 bg-background/50 hover:bg-muted/10'}`}>
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 p-1 rounded-full text-[10px]"><CheckCircle className="w-3 h-3" /></span>
                            <span className="tracking-tight">Resume/CV (100%)</span>
                          </span>
                          <ChevronDown className={`w-4 h-4 text-muted-foreground transition-transform duration-300 ${isOpen ? 'rotate-180 text-primary' : ''}`} />
                        </button>
                        {isOpen && (
                          <div className="px-4 pb-4 pt-0 text-[12px] text-muted-foreground space-y-2 select-text animate-in slide-in-from-top-1 duration-150">
                            <div className="flex items-start gap-2">
                              <span className="text-primary font-black mt-0.5">•</span>
                              <p className="font-extrabold text-foreground">CV telah diunggah</p>
                            </div>
                          </div>
                        )}
                      </div>
