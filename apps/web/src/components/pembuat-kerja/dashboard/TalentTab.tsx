...
const getRolesForCard = (title: string): string[] => {
  ...
  const splitRoles = cleaned.split(/\s*(?:&|\/|and|,)\s*/i).map(r => r.trim()).filter(Boolean);
  ...
  if (lower.includes('backend') || lower.includes('node') || lower.includes('laravel')) {
    return [single, 'Web Developer', 'Software Engineer'];
  }
  if (lower.includes('fullstack') || lower.includes('full stack')) {
    return [single, 'Web Developer', 'Software Engineer'];
  }
  if (lower.includes('designer') || lower.includes('ui') || lower.includes('ux') || lower.includes('figma')) {
    return [single, 'UI/UX Designer', 'Graphic Designer'];
  }
  if (lower.includes('writer') || lower.includes('content') || lower.includes('seo')) {
    return [single, 'SEO Specialist', 'Copywriter'];
  }
  if (lower.includes('marketing') || lower.includes('marketer') || lower.includes('social media')) {
    return [single, 'Social Media Specialist', 'Digital Marketer'];
  }
  if (lower.includes('admin') || lower.includes('office') || lower.includes('secretary')) {
    return [single, 'Office Administrator', 'Data Entry'];
  }
  if (lower.includes('hr') || lower.includes('recruiter') || lower.includes('human resource')) {
    return [single, 'HR Specialist', 'Talent Acquisition'];
  }
  if (lower.includes('sales') || lower.includes('business development') || lower.includes('bizdev')) {
...
export default function TalentTab({ updateTabInUrl }: TalentTabProps) {
  const {
    unlockedTalents,
    unlockTalent,
    favoriteTalents,
  ...
  const filteredTalents = mockTalents.filter((talent) => {
    ...
    if (talentSearch.trim()) {
      ...
      const titleMatch = talent.title.toLowerCase().includes(q) || talent.name.toLowerCase().includes(q);
    ...
    if (selectedProvince) {
      const matchProv = talent.location.toLowerCase().includes(selectedProvince.toLowerCase());
      if (!matchProv) {
    ...
    if (selectedRegency && !talent.location.toLowerCase().includes(selectedRegency.toLowerCase())) {
      return false;
    }
    ...
    // Education Filter
    if (cEducation !== 'Semua') {
      if (cEducation === 'SMA/SMK' && talent.education !== 'SMA/SMK') return false;
    ...
    if (selectedWorkOptions.length > 0) {
      const talentWorkOption = talent.willingToRelocate 
        ? 'Onsite' 
        : (parseInt(talent.id.replace(/\D/g, '') || '0', 10) % 2 === 0 ? 'Remote' : 'Hybrid');
    ...
    if (candShowWithPhoto && (!talent.avatar || talent.avatar.includes('default'))) return false;
              ...
              <div className="relative">
                ...
                {!unlockedTalents.includes(viewingTalentProfile.id) && (
                  <div className="absolute inset-0 bg-card/85 backdrop-blur-xs z-10 flex flex-col items-center justify-center p-4 rounded-xl border border-rose-500/30">
                    ...
                    <p className="text-xs font-bold text-foreground">Referensi Pekerjaan Minat Terkunci</p>
                    <p className="text-[10px] text-muted-foreground max-w-[280px] text-center mt-1">
                      Buka profil talent ini untuk melihat rincian minat dan preferensi pekerjaan.
                    </p>
                  </div>
                )}

                <div className={`space-y-6 ${!unlockedTalents.includes(viewingTalentProfile.id) ? 'filter blur-[1.5px] select-none pointer-events-none' : ''}`}>
                  {/* Row 1: Bidang Minat */}
            ...
            <div className="bg-card border border-border rounded-2xl p-6 shadow-sm space-y-4 relative overflow-hidden">
              ...
              {unlockedTalents.includes(viewingTalentProfile.id) ? (
                ...
                <div className="relative py-1 space-y-4">
                  {/* Mock content with Lock Icons & Titles */}
                    ...
                    <div className="flex items-start gap-3 opacity-60">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          ...
                          <span className="font-bold text-muted-foreground italic">Resume Terkunci</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 opacity-60">
                      <Globe className="w-4 h-4 text-slate-400 mt-0.5 shrink-0" />
                      <div>
                        <span className="text-[10px] text-muted-foreground uppercase block font-black tracking-wide">Website / Portofolio</span>
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <Lock className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                          <span className="font-bold text-muted-foreground italic">Link Terkunci</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-start gap-3 opacity-60">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5">
                          <Lock className="w-3.5 h-3.5 text-amber-500 shrink-0" />
                          <span className="font-bold text-muted-foreground italic">LinkedIn Terkunci</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="absolute inset-0 bg-card/65 backdrop-blur-[1.5px] z-10 flex flex-col items-center justify-center p-4 rounded-xl border border-border/50">
                    <Lock className="w-6 h-6 text-amber-500 mb-1" />
                    <p className="text-[10px] font-black text-foreground uppercase tracking-wide">Konten Terkunci</p>
                    <p className="text-[9px] text-muted-foreground max-w-[190px] mb-2 text-center">Unlock profile dengan koin untuk membuka resume & tautan</p>
                    <Button
                      onClick={(e) => {
                        e.stopPropagation();
                        const success = unlockTalent(viewingTalentProfile.id);
                        if (success) {
                          alert(`Berhasil membuka profil ${viewingTalentProfile.name}! Saldo berkurang 10 koin.`);
                        } else {
                          alert('Koin Anda tidak cukup untuk membuka profil ini! Silakan lakukan top up di tab Koin.');
                          updateTabInUrl('coin-credit');
                        }
                      }}
                      size="sm"
                      className="h-6.5 text-[9px] font-extrabold uppercase gap-1 bg-linear-to-r from-amber-500 to-orange-500 hover:bg-amber-600 text-white rounded-xl border-none shadow-sm shadow-amber-500/10 cursor-pointer active:scale-95 transition-all px-2.5"
                    >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
      ...
      <div className="h-[3240px] w-full space-y-4 lg:col-span-3">
              ...
              >
                {tab === 'All'
                  ...
                  : tab === 'Favorite'
                    ...
                    : <span className="flex items-center gap-1"><Unlock className="w-3 h-3" /> Cv terbuka</span>}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {paginatedTalents.map((talent) => {
              ...
              return (
                <Card
                  key={talent.id}
                  className="border border-border/80 hover:border-emerald-500/30 transition-all duration-300 rounded-2xl overflow-hidden shadow-sm bg-card text-card-foreground hover:shadow-md flex flex-col justify-between h-[320px] relative group"
                >
                  <div className="p-5 relative flex-1 flex flex-col justify-between">
                    ...
                    <div className="flex flex-col items-center text-center">
                      <div className="relative">
                        {talent.avatar && !talent.avatar.includes('default-avatar') && !talent.avatar.includes('placeholder') ? (
                      ...
                      <div className="flex flex-wrap items-center justify-center gap-1.5 mt-1.5 max-w-full h-[48px] overflow-hidden content-start">
                        {getRolesForCard(talent.title)
                          ...
                          .map((role: string, roleIdx: number) => (
                            ...
                            >
                              {roleIdx === 0 && <Briefcase className="w-3 h-3 shrink-0" />}
                      ...
                      <div className="w-full mt-3.5 space-y-1.5">
                        <div className="flex justify-between text-[9px] font-bold text-muted-foreground uppercase tracking-wider">
                          ...
                          <span className="text-emerald-500">{talent.profileScore}%</span>
                        </div>
                  ...
                  <div className="p-4 border-t border-border/60 flex items-center justify-between gap-3 bg-muted/20 dark:bg-muted/10 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="p-1.5 rounded-xl hover:bg-muted text-muted-foreground hover:text-primary transition-all cursor-pointer border border-border flex items-center justify-center bg-transparent"
                      ...
                      {isUnlocked ? (
                        <div className="bg-emerald-500/10 text-emerald-650 border border-emerald-500/25 font-extrabold text-[10px] h-7 px-3 flex items-center gap-1 rounded-xl shrink-0">
                          <Unlock className="h-3 w-3" />
        ...
        {totalTalentPages > 1 && (
          <div className="flex justify-center items-center gap-2 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800">
            <Button
              variant="outline"
              size="sm"
            ...
            <span className="text-xs text-slate-500 font-semibold px-2">
              Halaman {talentPage} dari {totalTalentPages}
            </span>
            <Button
              variant="outline"
              size="sm"
